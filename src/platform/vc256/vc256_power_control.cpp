    ...
    VRPowerControl(ioContext, conn, configFilePath, node, appState)
{
    ...
    // Seed the standby guard from hardware. Runs last so the power state and
    // host state interface are already established before a latched loss can
    // force PowerState::off.
    refreshStandbyLostFromHardware();
}
...
{
    // Re-sample the witness rather than trusting the latched flag alone, in
    // case the de-assert edge was missed.
    refreshStandbyLostFromHardware();
...
bool VC256PowerControl::shouldIgnoreEvent(const std::string& signalName)
{
    ...
    if (stby && stby->gpioLine)
    {
        ...
        {
            // Standby state is unknown, so ignore this event rather than let a
            // possibly spurious IOX event drive a transition. Not treated as a
            // standby loss: StbyPwrOk is a direct ASPEED GPIO off the
            // sequencing IOX, so a failed read here is not evidence the rail
            // dropped, and markStandbyLost() is only cleared by an AC cycle.
            lg2::error(
                "Failed to read StbyPwrOk witness: {ERROR}. Ignoring {SIGNAL} event",
                "ERROR", e, "SIGNAL", signalName);
            return true;
        }
...
void VC256PowerControl::refreshStandbyLostFromHardware()
{
    if (stbyPowerLost)
    {
        return;
    ...
    if (!stby || !stby->gpioLine)
    {
        lg2::error("CRITICAL: StbyPwrOk not available, cannot sample witness");
        return;
    }

    try
    {
        if (stby->gpioLine.get_value() != stby->polarity)
        {
            lg2::error("StbyPwrOk reads de-asserted on sample");
            markStandbyLost();
        }
    }
    catch (const std::exception& e)
    {
        // Leave the flag as-is. A failed read is not evidence the rail
        // dropped, and markStandbyLost() is only cleared by an AC cycle.
        lg2::error("Failed to read StbyPwrOk witness: {ERROR}", "ERROR", e);
    }
}
